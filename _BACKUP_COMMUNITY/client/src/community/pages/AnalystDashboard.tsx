                    ...
                    <TabsContent value="payments">
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-in fade-in duration-500">
                            {pendingPayments.data?.length === 0 ? (
                                ...
                                pendingPayments.data?.map(item => (
                                    <Card key={item.id} className="bg-zinc-900/60 border-zinc-800 rounded-3xl overflow-hidden hover:border-emerald-500/30 transition-all">
                                        <CardHeader>
                                            <div className="flex items-center justify-between mb-2">
                                                ...
                                                <span className="text-xl font-black text-white">R$ {item.amount}</span>
                                            </div>
                                            <CardTitle className="text-lg font-bold truncate">{item.video.title}</CardTitle>
                                            <CardDescription className="text-zinc-500 text-xs truncate">{item.description}</CardDescription>
                                        </CardHeader>
                                        <CardContent className="space-y-4">
                                            <div className="p-3 bg-zinc-950/50 rounded-2xl border border-zinc-800 text-xs text-zinc-400">
                                                ID: <span className="text-zinc-500 font-mono">{item.stripePaymentIntentId}</span>
                                            </div>
                                                        ...
                                                        if (confirm("Deseja simular o Stripe avisando o site agora? Isso é o que acontecerá automaticamente no futuro.")) {
                                                            fetch("/api/community/webhooks/stripe", {
                                                                ...
                                                                body: JSON.stringify({
                                                                    ...
                                                                    data: { object: { client_reference_id: item.id.toString() } }
                                                                })
                                                            }).then(() => {
                            ...
                            <div className="lg:col-span-1 space-y-4">
                                ...
                                <div className="space-y-3">
                                    {pendingVideos.data?.length === 0 ? (
                                        ...
                                        pendingVideos.data?.map(v => (
                                            <div
                                                key={v.id}
                                                onClick={() => setSelectedVideoId(v.id)}
                                                className={`p-4 rounded-xl border cursor-pointer transition-all duration-300 group ${selectedVideoId === v.id
                                                    ? "bg-emerald-500/10 border-emerald-500/50 shadow-[0_0_20px_rgba(16,185,129,0.1)]"
                                                    : "bg-zinc-900/60 border-zinc-800 hover:border-zinc-700"
                                                    }`}
                                            >
                                                <div className="flex justify-between items-start mb-2">
                                                    <h3 className="font-bold text-zinc-200 group-hover:text-white transition-colors capitalize line-clamp-1">{v.title}</h3>
                                                    <Clock className="w-4 h-4 text-zinc-600" />
                                                </div>
                                                <div className="text-xs text-zinc-500 flex items-center gap-2">
                                                    <VideoIcon className="w-3 h-3" />
                                                    <span>Submetido {new Date(v.createdAt!).toLocaleDateString()}</span>
                                                </div>
                                            </div>
                                        ))
                                    )}
